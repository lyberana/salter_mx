...
export async function GET(req: NextRequest) {
  ...
  try {
    const results = await repo.searchRemitentes(q);
    log.info({ event: "remitentes.search", query: q, count: results.length });
    return NextResponse.json({ data: results, meta: { total: results.length } });
  } catch (err) {
    log.error({ err }, "Error searching remitentes");
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await service.createRemitente(parsed.data);
    if (!result.ok) {
      return NextResponse.json(
    ...
    log.info({ event: "remitente.created", id: result.data.id });
    return NextResponse.json({ data: result.data }, { status: 201 });
  } catch (err) {
    log.error({ err }, "Error creating remitente");
    return NextResponse.json(
      { data: null, errors: [{ code: "SERVER_ERROR", message: "Error interno" }] },
      { status: 500 }
    );
